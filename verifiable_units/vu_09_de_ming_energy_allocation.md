...
在非平稳的多臂老虎机决策任务中，使用“惊奇驱动信念更新”的自适应策略（De-Ming）是否比固定最优策略、固执单一策略和随机策略表现出更低的累积后悔值和更快的环境变化检测？

### 3.2 参与者

- 80 名健康成人，随机分配到四组：De-Ming 自适应组、Dogmatic 固定组、Stubborn 固执组、Random 随机组（或通过指导语操纵策略）。
- 排除已参与类似任务或熟悉多臂老虎机最优解的被试。

### 3.3 任务与条件

**任务**：网页版 4 选项决策任务，共 200 试次。每试次选择一个选项，获得带有噪声的奖励反馈。

**非平稳设计**：
- 前 100 试次：选项 A/B 期望奖励较高（例如 0.7/0.6），C/D 较低（0.3/0.2）。
- 第 100 试次后：奖励分布翻转，C/D 变为高奖励（0.7/0.6），A/B 变为低奖励（0.3/0.2）。

**四组策略**（与仿真对应）：
1. **De-Ming 组**：无预设，被试自由学习；记录其选择、反应时、瞳孔扩张或心率作为惊奇信号。
2. **Dogmatic 组**：指导语要求“前 100 试次找到最优选项后坚持到底”。
3. **Stubborn 组**：指导语要求“始终选择你最初认为最好的选项”。
4. **Random 组**：指导语要求“尽可能随机选择”。

### 3.4 主要测量

1. **累积后悔值**：每试次最优期望奖励与实际获得奖励之差的累加。
2. **切换潜伏期**：环境翻转后，被试从旧最优选项切换到新最优选项所需的试次数。
3. **生理惊奇指标**：瞳孔扩张峰值、心率加速（作为“明”的外周代理）。
4. **策略自陈**：任务后问卷询问“你何时意识到规则变了？”。

### 3.5 统计假设

- $H_0$：四组在累积后悔和切换潜伏期上无显著差异。
- $H_1$：De-Ming 组累积后悔显著低于其他三组，切换潜伏期显著短于 Dogmatic/Stubborn 组。
- 预期效应量：Cohen's $d \approx 0.7$–$0.9$（大）。
