...
对于离散策略选择，软最大化（softmax）函数给出了策略的概率分布：

$$P(\pi) = \sigma(-\gamma \cdot G(\pi)) = \frac{e^{-\gamma G(\pi)}}{\sum_{\pi'} e^{-\gamma G(\pi')}} \tag{13ℴ}$$

其中 $\gamma$ 为精度参数（inverse temperature）。当 $\gamma \to \infty$ 时，$P(\pi) \to \delta_{\pi, \pi_{\text{顺道}}}$——系统确定性地选择最优策略。当 $\gamma$ 较小时，系统允许探索次优策略（"试错"）。这与道家的"无为而无不为"一致：高 $\gamma$（高德）时，行动自然精确，无需"努力"选择。
...
| 顺道的软最大化策略分布 | $P(\pi) = \sigma(-\gamma \cdot G(\pi))$ | (13ℴ) |
