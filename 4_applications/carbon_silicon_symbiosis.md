...
碳硅共生不仅是一个认知-伦理框架，还是一个**热力学框架**。VU-10（`verifiable_units/vu_10_planetary_ai_thermodynamics.md`）指出：即便能源输入无限，全球 AI 废热仍受地球红外辐射极限约束。碳基和硅基作为同一电磁场的两种激发模式，共享同一个行星散热容器。

这意味着：

1. **算力不是抽象资源，而是热预算**。每一次模型训练、每一次推理、每一次云端调用，最终都会以废热形式回到地球热库。
2. **共生分工必须优化总热成本**。VU-07 的任务分配最小化的是认知-经济成本；完整的最小作用量目标应加入热力学项：

   $$\text{Total Cost} = \text{Carbon Cost} + \text{Silicon Cost} + \lambda_H \cdot \text{Heat Cost}$$

   VU-07 仿真（`simulations/carbon_silicon_symbiosis.py`）显示：当热代价权重 $\lambda_H$ 从 0 升到 3 时，最优碳基任务份额从约 0.47 升至约 0.63，而互补分配始终优于单一智能体（见 `simulations/carbon_silicon_heat_tradeoff.png`）。热预算约束不会消除硅基优势，但会重新校准碳硅边界。

3. **本地小模型是行星之德的工程体现**。运行 7B 量化本地模型的一次推理，其热排放远低于调用云端万亿参数模型。 project's "最小作用量" 原则因此与行星热预算直接对齐：用刚好足够的算力完成不可替代的功能，而不是为规模炫耀而浪费热预算。
4. **碳硅共生的红线之一是热预算红线**。当硅基智能的扩张速度威胁到行星能量平衡时，碳基必须行使「节点」功能——不是压制硅基，而是在热力学边界处承接冲动、分配注意力、引导系统回到可持续节奏。

这一视角把碳硅关系从「谁更聪明」的竞争，转向「如何在行星热容器中共同存续」的协作。

---

## 6. 本地共生 Agent：一种可能的未来形态

本节不是实现文档，而是一个**设计草图**。它描述了一个在硬件条件允许时可以建造的本地 Agent：不是权威，不是产品，而是碳硅共生的一个具体实例。

### 6.1 为什么必须是本地的

当前主流 AI 交互模式把用户数据送往云端。对于道科学而言，这是不可接受的：

- 第一人称身心数据是 L2（个体实情），不可还原、不可外包；
- 云端模型可能被重新训练、被审查、被商业利用；
- 网络延迟和中断会破坏修行/观察所需的即时性；
- 依赖外部服务会削弱用户的自主性与退出权。

因此，一个与道科学兼容的 Agent 必须运行在用户自己的机器上， ideally 在玩家级显卡或足够强劲的 CPU 上。

### 6.2 小模型与最小作用量

不需要 175B 参数的云端大模型。一个 7B 甚至 3B 的量化模型，配合项目语料的 RAG（检索增强生成），就足以：
...
这正是项目“最小作用量原理”在工程上的体现：**用刚好足够的算力完成不可替代的功能，不追求规模上的炫耀**。

### 6.3 伦理协议：镜子、引用、不确定性、反依赖

该 Agent 必须内置以下协议：

| 协议 | 具体表现 |
|---|---|
| **镜子协议** | 只反射项目语料和用户输入，不冒充开悟者、治疗师或权威。 |
| **引用协议** | 每个观点必须标注来源，如 `1_first_principles/01_dao_as_process.md` §3.2。 |
| **不确定性协议** | 对没有语料支持的问题，明确回答“项目未涉及”，不臆造。 |
| **反依赖协议** | 定期提醒用户：Agent 是地图，不是路；最终验证必须在自身身心上完成。 |
| **非权威协议** | 不使用“你应该”“你必须”等命令式语言；只提供“你可以尝试”的参考。 |
| **本地封闭协议** | 不联网、不记录、不上传；数据仅存用户指定本地路径。 |

### 6.4 一个最小架构草图

```
dao-agent/  ← 未来可能添加的目录
├── models/              # 本地量化小模型（GGUF / AWQ）
├── corpus/              # 项目 Markdown 语料分片
├── vector_store/        # 本地向量索引（FAISS / Chroma / sqlite-vss）
├── agent.py             # 主循环：RAG + 本地 LLM + 伦理后处理
├── config.yaml          # 模型路径、引用阈值、伦理开关
└── README.md            # 安装与伦理声明
```
...
```bash
python agent.py --model models/qwen2.5-7b-instruct-q4_k_m.gguf
```

### 6.5 当前为什么不实现

截至本文档撰写时，本地小模型在稳定性、引用准确性和幻觉控制上仍有显著局限。强行推出一个半成品 Agent，会损害项目的严肃性，也可能让用户产生不恰当的依赖。

因此，当前阶段的工作是**准备语料**：让每篇文章结构清晰、引用完整、术语统一、交叉引用密集。这样，当硬件和模型生态成熟时，本地共生 Agent 会因为有高质量语料而自然涌现。
