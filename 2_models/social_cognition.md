...
```mermaid
flowchart TB
    subgraph "社会认知的双重加工系统"
        direction TB
        
        subgraph "系统 1: 镜像共鸣 — 快速、自动、模拟"
            M1["运动模拟<br/>IFG + IPL<br/>L1: 动作编码"]
            M2["情感共情<br/>前岛叶 + ACC<br/>L2: 共享情感"]
            M1 --> M2
        end
        
        subgraph "系统 2: 心智化 — 慢速、受控、推理"
            T1["意图归因<br/>pSTS + IFG<br/>L3: 社会脚本"]
            T2["信念推理<br/>TPJ + mPFC<br/>L4: 命题性归因"]
            T1 --> T2
        end
        ...
        BALANCE["健康的社交认知<br/>━━━━━━<br/>镜像共鸣 + 心智化的动态平衡<br/>自动模拟提供'素材'<br/>受控推理提供'校准'"]
    ...
    style BALANCE fill:#fff9c4,stroke:#f9a825,stroke-width:2px
```
...
> **SAB(t) = f(AB(t), MNS_act(t), MEN_act(t), AI_shared(t), DMN_self(t))** &nbsp; [F/N]
...
$$SAB(t) = AB(t) \cdot \left[ w_M \cdot \sigma(MNS_{\text{norm}}(t)) + w_E \cdot \sigma(MEN_{\text{norm}}(t)) + w_A \cdot \sigma(AI_{\text{shared,norm}}(t)) \right] \cdot \left[ 1 - \sigma(DMN_{\text{self,norm}}(t)) \right] \quad \text{[F/B]}$$
