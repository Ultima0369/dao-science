...
$$\tau_a \frac{da}{dt} = -a(t) + w_{sa} \cdot S(t) + w_{aa} \cdot f(a) - w_{pa} \cdot p(t) + \sigma_a \cdot \eta_a(t)$$

$$\tau_p \frac{dp}{dt} = -p(t) + w_{sp} \cdot S(t) + w_{ap} \cdot a(t-\delta) + \sigma_p \cdot \eta_p(t)$$
...
$$\frac{dA}{dt} = \alpha_B \cdot S(t) - \alpha_T \cdot P(t) \cdot A(t) - \tau_A \cdot A(t)$$

其中：
- $A(t)$是杏仁核的激活水平
- $S(t)$是刺激强度（包括低通路和高通路的输入）
- $P(t)$是前额叶调控的强度
- $\alpha_B$是自下而上（bottom-up）的驱动系数
- $\alpha_T$是自上而下（top-down）的调控系数
- $\tau_A$是杏仁核激活的内在衰减率

前额叶调控的动态为：

$$\frac{dP}{dt} = \beta \cdot A(t - \delta) - \tau_P \cdot P(t)$$

其中$\delta$是前额叶对杏仁核信号的反应延迟（约100-200ms），$\beta$是前额叶对杏仁核信号的响应增益，$\tau_P$是前额叶活动的衰减率。
...
$$A^* = \frac{\alpha_B \cdot S}{\alpha_T \cdot P^* + \tau_A}$$
$$P^* = \frac{\beta \cdot A^*}{\tau_P}$$

联立求解得到：

$$A^* = \frac{\alpha_B \cdot S \cdot \tau_P}{\alpha_T \cdot \beta \cdot A^* + \tau_A \cdot \tau_P}$$

这给出了一个关于$A^*$的二次方程，其稳定解对应于情绪反应的适应性水平。当自下而上的驱动过强（$\alpha_B \cdot S$过大）或自上而下的调控过弱（$\alpha_T \cdot \beta$过小）时，系统可能进入不稳定的高激活状态——即杏仁核劫持。
