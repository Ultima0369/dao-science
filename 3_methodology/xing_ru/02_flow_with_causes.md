...
> 本文是 Project Dao.Science 实践方法论（`3_methodology/`）"行入四行"系列的第二篇。上一篇：`01_embrace_suffering.md`（报冤行）。下一篇：`03_seek_nothing.md`（无所求行）。建议与 `2_models/attention_model.md`（注意力动力学模型）配合阅读。