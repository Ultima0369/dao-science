...
- **理入 = 更新生成模型的最高层级先验**：在预测编码框架中，最高层级的先验（highest-level priors）编码了系统关于世界和自身的最抽象、最基础的信念（Friston, 2010; Clark, 2016）。这些先验约束了所有下级层级的推理过程。改变最高层级的先验——即改变关于"自我是什么"、"心智如何运作"、"实在的本质是什么"的基本信念——将对整个认知-情感系统产生级联效应（cascade effects）。
...
**操作**：
1. 系统学习本项目第一性原理部分的五篇论文：
   - `01_dao_as_process.md`：理解"道"作为预期自由能上的梯度流
   - `02_one_as_bandwidth.md`：理解"一"作为觉知带宽（AB = 1 - [R_DMN - R_0]/[R_max - R_0]）
   - `03_map_not_territory.md`：理解心智内容=表征≠实在（五传统收敛）
   - `04_philosophy_of_science.md`：理解科学知识的认知层级——为什么 L0-L7 框架可以作为"元方法论"
   - `05_first_person_epistemology.md`：理解第一人称认识论——为什么个体经验和觉知本身是科学不可还原的数据层
   （第四、五篇是方法论的——它们帮你理解"为什么这套框架在科学上是合法的"以及"为什么个体独特性必须被保留"。如果时间有限，可先跳过，直接进入前三篇。）
...
4. 建立"第一人称观察者"的操作身份：你不仅是被观察的心智内容的拥有者，也是观察这些内容的研究者。使用 `3_methodology/n_of_1_protocol.md` 中的结构化日志格式，使个人证据库从"零散感悟"升级为"可检验数据"。
...
### 5.3a 德作为精力经济学：先验重构的能量维度

"理入"的建立不仅是认知操作——它同时是一种**能量管理策略**。从"德"（De）的视角来看，"德"可以被精确地定义为**生成模型的精度校准状态**（$\Pi_{\text{gen}}$），而修德的过程就是**将有限的注意力/能量资源从高能耗低收益的精度配置转移到低能耗高收益的精度配置**。

"德"的三层结构（精力经济学视角）：

1. **第一层：识别能量泄漏（Energy Leak Detection）**。DMN 的反刍思维（"他为什么那样看我？""我是不是做错了什么？"）是认知系统最大的能量泄漏源——它持续消耗前额叶的代谢资源而不产生任何行动输出。从预测编码的视角，反刍的本质是**精度配置错误**：系统将过高的精度分配给了自我叙事先验（$\Pi_{\text{self-narrative}} \gg \Pi_{\text{self-narrative}}^*$），导致预测误差被异常放大，触发持续的、无解的信念更新循环。"理入"的第一步是**识别这些能量泄漏**——不是通过"压制念头"（那需要更多能量），而是通过"标记"（labeling）——"这是一个反刍叙事"——来下调其精度。

2. **第二层：重新分配注意力预算（Attention Budget Reallocation）**。将注意力从"无法改变的事"（他人的评价、过去的事件、未来的不确定性）转移到"可以改变的事"（此刻的行动、当下的身体状态、可操作的具体步骤）。这与"轻重缓急，暂时确定"的决策原则一致——在复杂、不确定的情境中，无法穷尽所有信息时，依据当前实情做出临时决策，并随时准备调整。这不是"放弃思考"，而是**将算力从无限递归的预测误差最小化循环中解放出来，投向有实际效用的行动**。

3. **第三层：建立"知止"的停止标准（Stopping Criteria）**。"德"的最高表现不是"能做什么"，而是"知道何时不做什么"——即"知止"（见 `4_applications/ai_governance.md`）。在认知层面，"知止"意味着：当一个思维循环的边际效用趋近于零（继续思考不再产生新的洞察），系统自动停止该循环并切换到其他任务或休息状态。这与最优停止理论（Optimal Stopping Theory; Shiryaev, 1978）一致——"德"作为内化的停止标准，使得系统不需要外部规则来告诉它"够了"。

从道家原典的视角，"德"的这三层结构对应于：
- **"慈"（compassion）** = 识别能量泄漏而不自我攻击（"我又在反刍了——没关系，标记，放下"）
- **"俭"（frugality）** = 重新分配注意力预算（"这个念头值得我花能量吗？不值得——放下"）
- **"不敢为天下先"（not presuming to be first）** = 建立知止的停止标准（"我不需要穷尽所有可能性——够了，行动"）

这三者构成了"德"作为精力经济学的完整操作框架：**不是通过意志力"压制"消耗，而是通过精度校准"消除"不必要的消耗。**
...
> 本文是 Project Dao.Science 实践方法论（`3_methodology/`）的第一篇，对应达摩"二入四行"体系中的"理入"。本文应与第一性原理部分（`1_first_principles/`）的六篇论文配合阅读，它们共同构成"理入"的概念基础。后续四篇论文（`xing_ru/01-04`）分别对应"四行"的实践操作，而 `n_of_1_protocol.md` 提供了把第一人称经验转化为可检验数据的操作协议。
>
...
> 下一篇：`3_methodology/n_of_1_protocol.md`（N-of-1 实践协议）。