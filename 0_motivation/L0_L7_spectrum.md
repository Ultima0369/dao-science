...
> 本文是 Project Dao.Science 的补充框架文件，与第一性原理部分（`1_first_principles/`）和应用部分（`4_applications/`）形成交叉引用。L0-L7 频谱为理解"道"（L0 的道体与 L1-L4 的道用）、"一"（L2 的个体实情与 L3 的文化承托）、"知止"（L4 的工程实现）提供了统一的认知地形图。
